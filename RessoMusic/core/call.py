...
class Call(PyTgCalls):
        ...
        try:
            ...
            try:
                if queued and "live_" in queued:
                    ...
                    run = await app.send_photo(chat_id=original_chat_id, photo=img, has_spoiler=True, caption=_["stream_1"].format(f"https://t.me/{app.username}?start=info_{videoid}", title[:23], check[0]["dur"], user), reply_markup=InlineKeyboardMarkup(button))
                    ...
                    await client.change_stream(chat_id, stream)
                    button = stream_markup(_, chat_id)
                    run = await app.send_photo(chat_id=original_chat_id, photo=config.STREAM_IMG_URL, has_spoiler=True, caption=_["stream_2"].format(user), reply_markup=InlineKeyboardMarkup(button))
                    db[chat_id][0]["mystic"] = run
                    ...
                    if videoid == "telegram" or videoid == "soundcloud":
                        photo_url = config.TELEGRAM_VIDEO_URL if video else config.TELEGRAM_AUDIO_URL
                        if videoid == "soundcloud": photo_url = config.SOUNCLOUD_IMG_URL
                        button = stream_markup(_, chat_id)
                        run = await app.send_photo(chat_id=original_chat_id, photo=photo_url, has_spoiler=True, caption=_["stream_1"].format(config.SUPPORT_GROUP, title[:23], check[0]["dur"], user), reply_markup=InlineKeyboardMarkup(button))
                        db[chat_id][0]["mystic"] = run
                    else:
                        img = await gen_thumb(videoid)
                        if not img: img = FALLBACK_IMG
    ...
    async def start(self):
        ...
        # 🔥 THE MASTER HACK: AUTO-NEXT LISTENER ACTIVATED! 🔥
    ...
    async def decorators(self):
        ...
        async def stream_end_handler1(client, update: Update):
            if type(update).__name__ not in ["StreamAudioEnded", "StreamVideoEnded"]:
                return
