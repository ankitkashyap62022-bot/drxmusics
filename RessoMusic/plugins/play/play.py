...
# Random emoji uthane ka function
def get_rand_emo():
...
# ==========================================
# 🎨 CUSTOM THUMBNAIL LOGIC (/setply - BY REPLYING)
# ==========================================
CUSTOM_THUMB = None
...
async def set_ply_cmd(client, message):
    global CUSTOM_THUMB
    # Check agar message kisi photo par reply hai
    if not message.reply_to_message or not message.reply_to_message.photo:
        return await message.reply_text(
            f"{get_rand_emo()} 𝘉𝘢𝘣𝘺, 𝘒𝘪𝘴𝘪 𝘗𝘩𝘰𝘵𝘰 𝘗𝘢𝘳 𝘙𝘦𝘱𝘭𝘺 𝘒𝘢𝘳𝘬𝘦 /𝘴𝘦𝘵𝘱𝘭𝘺 𝘓𝘪𝘬𝘩𝘰! 🎀"
        )
    
    # Photo ki File ID nikaalo aur save kar lo
    CUSTOM_THUMB = message.reply_to_message.photo.file_id
...
async def play_commnd(client, message: Message, _, chat_id, video, channel, playmode, url, fplay):
    ...
    # 🔥 HACKER POOKIE UI STRING (RANDOM EMOJI EVERY TIME)
    ...
    if audio_telegram:
        ...
        if await YouTube.exists(url):
            ...
            streamtype = "youtube"
            # 🔥 Setting Custom Thumb if available
