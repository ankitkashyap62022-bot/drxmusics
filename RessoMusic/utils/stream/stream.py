...
# ==========================================
# 🎨 CUSTOM THUMBNAIL & CAPTION LOGIC
# ==========================================
DATA_FILE = "stream_custom_data.json"
...
def save_custom_data(key, value):
...
async def get_caption(_, link, title, duration, user):
    data = get_custom_data()
    custom_html = data.get("caption_html")
    if custom_html:
        try:
...
async def get_photo(vidid=None):
    data = get_custom_data()
...
# --- SET PHOTO COMMAND ---
@app.on_message(filters.command(["setphoto", "setply"]) & filters.user(config.OWNER_ID))
async def set_photo_cmd(client, message: Message):
    if not message.reply_to_message or not message.reply_to_message.photo:
        return await message.reply_text("🎀 ʙᴀʙʏ, ᴋɪsɪ ᴘʜᴏᴛᴏ ᴘᴀʀ ʀᴇᴘʟʏ ᴋᴀʀᴋᴇ `/setphoto` ʟɪᴋʜᴏ!")

    photo_id = message.reply_to_message.photo[-1].file_id
    save_custom_data("photo_id", photo_id)
    await message.reply_photo(photo_id, caption="✅ **Cusom Stream Photo Updated!**\n\nAb stream me yehi photo aayegi.")

# --- SET STREAM CAPTION COMMAND ---
@app.on_message(filters.command("setstream") & filters.user(config.OWNER_ID))
async def set_stream_cmd(client, message: Message):
    if len(message.command) < 2:
        return await message.reply_text("Usage: `/setstream [Your formatted text with {0}, {1}, {2}, {3}]`")
    ...
    save_custom_data("caption_html", caption_html)
...
async def stream(
    ...
    if streamtype == "playlist":
        ...
        if await is_active_chat(chat_id):
            ...
            button = stream_markup(_, chat_id)
            img = await get_photo()
            if img == config.STREAM_IMG_URL:
                img = config.SOUNCLOUD_IMG_URL
            cap = await get_caption(_, config.SUPPORT_GROUP, title[:23], duration_min, user_name)
